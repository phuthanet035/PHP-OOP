...
// Create booking
$testRoom = $roomsAvail[0] ?? $manager->getAllRooms()[0];
...
$res = $manager->createBooking($bookingData);
assert($res['success'] === true);
$createdCode = $res['booking']->getBookingCode();
echo "  Booking created with code: " . $createdCode . "\n";

// Test conflict detection with overlapping time (15:00 - 17:00)
$overlapData = [
    'roomId' => $testRoom->getId(),
    'customerName' => 'Conflict Person',
    'customerPhone' => '0877776666',
    'customerEmail' => 'conflict@test.com',
    'organization' => 'Overlap Co',
...
$resOverlap = $manager->createBooking($overlapData);
assert($resOverlap['success'] === false);
echo "  [PASS] Conflict detection successfully blocked overlapping reservation!\n";

// Test cancel booking
$cancelRes = $manager->cancelBooking($createdCode);
assert($cancelRes === true);
